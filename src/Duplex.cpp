 ...
 */
void Duplex::clientDisconnect(WSclient_t * client, uint16_t code, char * reason, size_t reasonLen) {
    DEBUG_DUPLEX("[WS][%d][handleDuplex] clientDisconnect code: %u\n", client->num, code);
 ...
 */
uint8_t Duplex::createHeader(uint8_t * headerPtr, WSopcode_t opcode, size_t length, bool mask, uint8_t maskKey[4], bool fin) {
    uint8_t headerSize;
 ...
 * @return true if ok
 */
bool Duplex::sendFrameHeader(WSclient_t * client, WSopcode_t opcode, size_t length, bool fin) {
    uint8_t maskKey[4]                         = { 0x00, 0x00, 0x00, 0x00 };
    uint8_t buffer[DUPLEX_MAX_HEADER_SIZE] = { 0 };
 ...
 * @return true if ok
 */
bool Duplex::sendFrame(WSclient_t * client, WSopcode_t opcode, uint8_t * payload, size_t length, bool fin, bool headerToPayload) {
    if(client->tcp && !client->tcp->connected()) {
        DEBUG_DUPLEX("[WS][%d][sendFrame] not Connected!?\n", client->num);
        return false;
    }

    if(client->status != WSC_CONNECTED) {
        DEBUG_DUPLEX("[WS][%d][sendFrame] not in WSC_CONNECTED state!?\n", client->num);
        return false;
    }

    DEBUG_DUPLEX("[WS][%d][sendFrame] ------- send message frame -------\n", client->num);
    DEBUG_DUPLEX("[WS][%d][sendFrame] fin: %u opCode: %u mask: %u length: %u headerToPayload: %u\n", client->num, fin, opcode, client->cIsClient, length, headerToPayload);

    if(opcode == WSop_text) {
        DEBUG_DUPLEX("[WS][%d][sendFrame] text: %s\n", client->num, (payload + (headerToPayload ? 14 : 0)));
    }
...
#ifdef DUPLEX_USE_BIG_MEM
    // only for ESP since AVR has less HEAP
    // try to send data in one TCP package (only if some free Heap is there)
    if(!headerToPayload && ((length > 0) && (length < 1400)) && (GET_FREE_HEAP > 6000)) {
        DEBUG_DUPLEX("[WS][%d][sendFrame] pack to one TCP package...\n", client->num);
        uint8_t * dataPtr = (uint8_t *)malloc(length + DUPLEX_MAX_HEADER_SIZE);
        if(dataPtr) {
            memcpy((dataPtr + DUPLEX_MAX_HEADER_SIZE), payload, length);
    ...
    if(headerToPayload) {
        ...
        headerPtr = (payloadPtr + (DUPLEX_MAX_HEADER_SIZE - headerSize));
    } else {
        headerPtr = &buffer[0];
    }
    ...
    if(client->cIsClient && useInternBuffer) {
        ...
        if(headerToPayload) {
            dataMaskPtr = (payloadPtr + DUPLEX_MAX_HEADER_SIZE);
        } else {
    ...
    if(headerToPayload) {
        ...
        if(write(client, &payloadPtr[(DUPLEX_MAX_HEADER_SIZE - headerSize)], (length + headerSize)) != (length + headerSize)) {
            ret = false;
        }
    } else {
        // send header
    ...
    DEBUG_DUPLEX("[WS][%d][sendFrame] sending Frame Done (%luus).\n", client->num, (micros() - start));
 ...
 * @param client WSclient_t *  ptr to the client struct
 */
void Duplex::headerDone(WSclient_t * client) {
    client->status    = WSC_CONNECTED;
    client->cWsRXsize = 0;
    DEBUG_DUPLEX("[WS][%d][headerDone] Header Handling Done.\n", client->num);
 ...
 * @param client WSclient_t *  ptr to the client struct
 */
void Duplex::handleDuplex(WSclient_t * client) {
    if(client->cWsRXsize == 0) {
        handleDuplexCb(client);
    }
}
 ...
 * @param size
 */
bool Duplex::handleDuplexWaitFor(WSclient_t * client, size_t size) {
    if(!client->tcp || !client->tcp->connected()) {
        return false;
    }

    if(size > DUPLEX_MAX_HEADER_SIZE) {
        DEBUG_DUPLEX("[WS][%d][handleDuplexWaitFor] size: %d too big!\n", client->num, size);
        return false;
    }
    ...
    DEBUG_DUPLEX("[WS][%d][handleDuplexWaitFor] size: %d cWsRXsize: %d\n", client->num, size, client->cWsRXsize);
    readCb(client, &client->cWsHeader[client->cWsRXsize], (size - client->cWsRXsize), std::bind([](Duplex * server, size_t size, WSclient_t * client, bool ok) {
        DEBUG_DUPLEX("[WS][%d][handleDuplexWaitFor][readCb] size: %d ok: %d\n", client->num, size, ok);
        if(ok) {
            client->cWsRXsize = size;
            server->handleDuplexCb(client);
        } else {
            DEBUG_DUPLEX("[WS][%d][readCb] failed.\n", client->num);
            client->cWsRXsize = 0;
            // timeout or error
...
void Duplex::handleDuplexCb(WSclient_t * client) {
    if(!client->tcp || !client->tcp->connected()) {
        return;
    }
    ...
    if(!handleDuplexWaitFor(client, headerLen)) {
        return;
    }
    ...
    DEBUG_DUPLEX("[WS][%d][handleDuplex] ------- read massage frame -------\n", client->num);
    DEBUG_DUPLEX("[WS][%d][handleDuplex] fin: %u rsv1: %u rsv2: %u rsv3 %u  opCode: %u\n", client->num, header->fin, header->rsv1, header->rsv2, header->rsv3, header->opCode);
    DEBUG_DUPLEX("[WS][%d][handleDuplex] mask: %u payloadLen: %u\n", client->num, header->mask, header->payloadLen);

    if(header->payloadLen > DUPLEX_MAX_DATA_SIZE) {
        DEBUG_DUPLEX("[WS][%d][handleDuplex] payload too big! (%u)\n", client->num, header->payloadLen);
    ...
    if(header->payloadLen > 0) {
        ...
        if(!payload) {
            DEBUG_DUPLEX("[WS][%d][handleDuplex] to less memory to handle payload %d!\n", client->num, header->payloadLen);
            clientDisconnect(client, 1011);
            return;
        }
        readCb(client, payload, header->payloadLen, std::bind(&Duplex::handleDuplexPayloadCb, this, std::placeholders::_1, std::placeholders::_2, payload));
    } else {
        handleDuplexPayloadCb(client, true, NULL);
    }
}

void Duplex::handleDuplexPayloadCb(WSclient_t * client, bool ok, uint8_t * payload) {
    WSMessageHeader_t * header = &client->cWsHeaderDecode;
    if(ok) {
        if(header->payloadLen > 0) {
        ...
        switch(header->opCode) {
            case WSop_text:
                DEBUG_DUPLEX("[WS][%d][handleDuplex] text: %s\n", client->num, payload);
                // no break here!
            case WSop_binary:
            ...
            case WSop_ping:
                // send pong back
                DEBUG_DUPLEX("[WS][%d][handleDuplex] ping received (%s)\n", client->num, payload ? (const char *)payload : "");
                sendFrame(client, WSop_pong, payload, header->payloadLen);
                messageReceived(client, header->opCode, payload, header->payloadLen, header->fin);
                break;
            case WSop_pong:
                DEBUG_DUPLEX("[WS][%d][handleDuplex] get pong (%s)\n", client->num, payload ? (const char *)payload : "");
...
#endif
                DEBUG_DUPLEX("[WS][%d][handleDuplex] get ask for close. Code: %d", client->num, reasonCode);
                if(header->payloadLen > 2) {
                    DEBUG_DUPLEX(" (%s)\n", (payload + 2));
                } else {
                    DEBUG_DUPLEX("\n");
        ...
        handleDuplexWaitFor(client, 2);
#endif

    } else {
        DEBUG_DUPLEX("[WS][%d][handleDuplex] missing data!\n", client->num);
        free(payload);
        clientDisconnect(client, 1002);
    }
}
 ...
 */
String Duplex::acceptKey(String & clientKey) {
 ...
 */
String Duplex::base64_encode(uint8_t * data, size_t length) {
 ...
 * @return true if ok
 */
bool Duplex::readCb(WSclient_t * client, uint8_t * out, size_t n, WSreadWaitCb cb) {
#if(DUPLEX_NETWORK_TYPE == NETWORK_ESP8266_ASYNC)
    if(!client->tcp || !client->tcp->connected()) {
        return false;
    }
    ...
    size_t len;
    DEBUG_DUPLEX("[readCb] n: %zu t: %lu\n", n, t);
    while(n > 0) {
        if(client->tcp == NULL) {
            DEBUG_DUPLEX("[readCb] tcp is null!\n");
            if(cb) {
                cb(client, false);
            }
            return false;
        ...
        if(!client->tcp->connected()) {
            DEBUG_DUPLEX("[readCb] not connected!\n");
            if(cb) {
                cb(client, false);
            }
            return false;
        ...
        if((millis() - t) > DUPLEX_TCP_TIMEOUT) {
            DEBUG_DUPLEX("[readCb] receive TIMEOUT! %lu\n", (millis() - t));
            if(cb) {
                cb(client, false);
            }
            return false;
...
#if(DUPLEX_NETWORK_TYPE == NETWORK_ESP8266)
            delay(0);
#endif
            continue;
        }
 ...
 * @return bytes send
 */
size_t Duplex::write(WSclient_t * client, uint8_t * out, size_t n) {
    if(out == NULL)
        return 0;
    if(client == NULL)
        return 0;
    ...
    size_t len      = 0;
    size_t total    = 0;
    DEBUG_DUPLEX("[write] n: %zu t: %lu\n", n, t);
    while(n > 0) {
        if(client->tcp == NULL) {
            DEBUG_DUPLEX("[write] tcp is null!\n");
            break;
        }

        if(!client->tcp->connected()) {
            DEBUG_DUPLEX("[write] not connected!\n");
            break;
        }

        if((millis() - t) > DUPLEX_TCP_TIMEOUT) {
            DEBUG_DUPLEX("[write] write TIMEOUT! %lu\n", (millis() - t));
            break;
        }
        ...
        if(len) {
            ...
            n -= len;
            total += len;
            //DEBUG_DUPLEX("write %d left %d!\n", len, n);
        } else {
            //DEBUG_DUPLEX("write %d failed left %d!\n", len, n);
        }
#if(DUPLEX_NETWORK_TYPE == NETWORK_ESP8266)
        delay(0);
#endif
...
size_t Duplex::write(WSclient_t * client, const char * out) {
    if(client == NULL)
        return 0;
    if(out == NULL)
        return 0;
 ...
 */
void Duplex::enableHeartbeat(WSclient_t * client, uint32_t pingInterval, uint32_t pongTimeout, uint8_t disconnectTimeoutCount) {
    if(client == NULL)
        return;
 ...
 * @param client WSclient_t *
 */
void Duplex::handleHBTimeout(WSclient_t * client) {
